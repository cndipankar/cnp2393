...
if (! class_exists('CSF_Shortcoder')) {
    ...
    // constans
        ...
        public function __construct($key, $params = [])
        {
            ...
            add_action('admin_footer', [ &$this, 'add_shortcode_modal' ]);
            add_action('customize_controls_print_footer_scripts', [ &$this, 'add_shortcode_modal' ]);
            add_action('wp_ajax_csf-get-shortcode-'. $this->unique, [ &$this, 'get_shortcode' ]);

            if (! empty($this->args['show_in_editor'])) {
                CSF::$shortcode_instances[] = wp_parse_args([ 'hash' => md5($key), 'modal_id' => $this->unique ], $this->args);

                // elementor editor support
                if (CSF::is_active_plugin('elementor/elementor.php')) {
                    add_action('elementor/editor/before_enqueue_scripts', [ 'CSF', 'add_admin_enqueue_scripts' ], 20);
                    add_action('elementor/editor/footer', [ &$this, 'add_shortcode_modal' ]);
    ...
    <?php
        ...
        public function get_shortcode()
        {
          ...
          //
            ...
            wp_send_json_success([ 'success' => true, 'content' => ob_get_clean() ]);
        }
        ...
        public static function once_editor_setup()
        {
            if (function_exists('register_block_type')) {
                add_action('init', [ 'CSF_Shortcoder', 'add_guteberg_block' ]);
            }

            if (csf_wp_editor_api()) {
                add_action('media_buttons', [ 'CSF_Shortcoder', 'add_media_buttons' ]);
            }
        }
        ...
        {
            wp_register_script('csf-gutenberg-block', CSF::include_plugin_url('assets/js/csf-gutenberg-block.js'), [ 'wp-blocks', 'wp-editor', 'wp-element', 'wp-components' ]);
            ...
            foreach (CSF::$shortcode_instances as $hash => $value) {
                register_block_type('csf-gutenberg-block/block-'. $hash, [
