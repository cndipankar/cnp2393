...
if (! class_exists('CSF_Options')) {
    ...
    // constans
        ...
        public function __construct($key, $params = [])
        {
            ...
            add_action('wp_ajax_csf_'. $this->unique .'_ajax_save', [ &$this, 'ajax_save' ]);
        ...
        public function ajax_save()
        {
            if (! empty($_POST['data'])) {
                ...
                if (wp_verify_nonce(csf_get_var('csf_options_nonce'. $this->unique), 'csf_options_nonce')) {
                    ...
                    wp_send_json_success([ 'success' => true, 'notice' => $this->notice, 'errors' => $this->errors ]);
                }
            }

            wp_send_json_error([ 'success' => false, 'error' => esc_html__('Error while saving.', 'csf') ]);
        }

        // get default value
        public function get_default($field, $options = [])
        {
        ...
        public function set_options()
        {
            if (wp_verify_nonce(csf_get_var('csf_options_nonce'. $this->unique), 'csf_options_nonce')) {
                ...
                if (! empty($transient['csf_import_data'])) {
                    ...
                    $request = (is_array($import_data)) ? $import_data : [];
        ...
        public function add_admin_menu()
        {
            ...
            if ($menu_type === 'submenu') {
                $menu_page = call_user_func('add_submenu_page', $menu_parent, $menu_title, $menu_title, $menu_capability, $menu_slug, [ &$this, 'add_options_html' ]);
            } else {
                $menu_page = call_user_func('add_menu_page', $menu_title, $menu_title, $menu_capability, $menu_slug, [ &$this, 'add_options_html' ], $menu_icon, $menu_position);
