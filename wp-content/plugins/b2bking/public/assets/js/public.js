	...
	//	});
		...
		function setHoverColorTable(quantity = 'no'){
			...
			if ($('.b2bking_shop_table').attr('class') !== undefined){
				...
				$('.b2bking_tiered_active_price').text($('.summary .b2bking_tiered_range_replaced:first').text().replace(' – ', ' - ').split(' - ')[1]);				// if can't be found,
				if ($('.summary .b2bking_tiered_range_replaced:first').text().split(' – ')[1] === undefined){
					$('.b2bking_tiered_active_price').text($('.b2bking_tiered_range_replaced:first').text().replace(' – ', ' - ').split(' - ')[1]);
				}
				...
				if (parseInt(b2bking_display_settings.productid) !== 0){
					...
					if (rangereplaced !== ''){
						$('.b2bking_tiered_active_price').text(rangereplaced.replace(' – ', ' - ').split(' - ')[1]);
					}
				}
